    ...
    for target in IPNetwork(target_ip):
        try:
            ...
            for one_port in all_open_ports:
                ...
                try:
                    db_Connection(response_for_db.get("ip", "none"), response_for_db.get("redirected_site", "none"), int(
                        response_for_db.get("port", "none")), response_for_db.get("source_code_check", "none"))
...
def db_Connection(target, web_page, port, login_page_result):
    ...
    cursor.execute("INSERT INTO founded_sites (host, page_url, port , login_page_result) VALUES ( %s , %s , %s , %s )",
                   (target, web_page, port, login_page_result))
